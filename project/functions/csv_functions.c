...
int parse_line(char *line, char *choice, float *temperature, char **rooms) {
  ...
  if (choice == NULL) {
    ...
    if (!found && i < MAX_ROOMS && room_name[0] != '\0') {
      ...
      if (rooms[j] == NULL) {
        print_error(__FILE__, __FUNCTION__, __LINE__, "Memory allocation failure\n");
        return EXIT_FAILURE;
      }
    }
...
FILE *open_file(char *filename, char *mode) {
  ...
  if (file == NULL) {
    char *err_msg;
    asprintf(&err_msg, "File %s couldn't be opened.\n", filename);
    print_error(__FILE__,__FUNCTION__,__LINE__, err_msg);
    free(err_msg);
    exit(EXIT_FAILURE);
  }
  return file;
...
int read_from_csv(char *str, char *filename, int line_length, char **rooms,
  ...
  /* Off-by-1 most likely here */
  while (!ret_val && !feof(file)) {
    if ((ret_val = read_line(str, LINE_LENGTH, file)) == 1) {
      fprintf(stderr, "File %s: bad data on line %d.\n", CSV_FILE, line_count);
      fclose(file);
      return EXIT_FAILURE;
    }
    line_count++;
    if (!ret_val && parse_line(str, selected_room, temperature, rooms)) {
      if (ret_val) {
        return EXIT_SUCCESS;
      } else {
        return EXIT_FAILURE;
